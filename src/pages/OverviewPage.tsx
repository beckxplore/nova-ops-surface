  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 md:mb-8 gap-3">
        <div>
          <h1 className="text-xl md:text-2xl font-bold tracking-tight text-white">Overview</h1>
          <p className="text-slate-400 mt-1 text-xs md:text-sm">
            {isLive ? 'Real-time organizational monitoring' : 'Connecting to data sources...'}
          </p>
        </div>
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ring-1 self-start ${
      ...
      {eco && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 md:p-4">
            <p className="text-[10px] md:text-xs text-slate-400 uppercase tracking-wider mb-1">Active Projects</p>
            <p className="text-xl md:text-2xl font-bold text-white">{eco.projects.length}</p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 md:p-4">
            ...
            <p className="text-xl md:text-2xl font-bold text-white">{eco.departments.length}</p>
            <p className="text-[10px] text-slate-600 mt-1">
              {eco.departments.filter((d: any) => d.project).length} locked to projects
            </p>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 md:p-4">
            <p className="text-[10px] md:text-xs text-slate-400 uppercase tracking-wider mb-1">Total Agents</p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 md:p-4">
            <p className="text-[10px] md:text-xs text-slate-400 uppercase tracking-wider mb-1">Orchestrator</p>
      ...
      {!eco && (
        <div className="bg-slate-900/50 border border-dashed border-slate-800 rounded-xl p-8 md:p-12 text-center mb-4 md:mb-6">
      ...
      {eco && eco.projects.length > 0 && (
        <div className="mb-4 md:mb-6">
          <h2 className="text-base md:text-lg font-semibold text-white mb-3">Active Projects</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            {eco.projects.map((proj: any) => (
              <div key={proj.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 md:p-5">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-medium text-white text-sm md:text-base">📁 {proj.name}</h3>
